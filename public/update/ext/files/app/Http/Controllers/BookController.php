...
//        $filter_active = $request->filter_active ?? 2;// 2 = all
        ...
        if(!is_null($request->filter_category)){
            $request->session()->put('book_filter_category',$request->filter_category);
        }
        $filter_category = $request->session()->get('book_filter_category') ?? "0";// 0= all
        $request->filter_category = $filter_category;

        // filter_active storage
        if(!is_null($request->filter_active) ){
            $request->session()->put('book_filter_active',$request->filter_active);
        }
        $filter_active = $request->session()->get('book_filter_active') ?? "2";
        $request->filter_active = $filter_active;

        $books = Book::orderBy('id','desc')->filters($request)->paginate(10);
        $categories = ContentCat::where('content_cat_type',2)->get();
        return view('admin.books.index',compact('books','categories','filter_title','filter_category','filter_active'));
    }
    ...
    public function create()
    {
        ...
        return view('admin.books.create',compact('content_cats','morefields'));
    }
    ...
    public function store(Request $request)
    {
        ...
        if($request->hasFile('image')){
            ...
            $imageName = 'book_'.$filename.'.'.$request->image->extension();
            $avatar_path = $request->image->move(public_path('uploads/books/'), $imageName);
        }
        ...
        if ($request->images){
            foreach($request->images as $file) {
                ...
                $avatar_path = $file->move(public_path('uploads/books/'.$book->id.'/'), $imageName);
                // // Create files
                if($avatar_path){
                    $f = File::create([
                        'file' => $imageName,
        ...
        if ($request->docs){
            foreach($request->docs as $f) {
                ...
                $avatar_path = $f->move(public_path('uploads/books/' . $book->id . '/'), $imageName);
                // // Create files
                if ($avatar_path) {
                    $ff = File::create([
                        'file' => $imageName,
        ...
        return redirect()->route('admin.books.index')->with('success','رکورد با موفقیت ایجاد شد');
    }
    ...
    public function edit(Book $book)
    {
        ...
        $content_cats = ContentCat::where('active',1)
            ->where('content_cat_type',2)
            ->get();
        $images = $this->getImages($book->id);
        $files = $this->getDocs($book->id);
        ...
        return view('admin.books.edit',compact('book','content_cats','files','images','morefields','morefields_full'));
    }
    public function image_set_main(Book $book,File $file){
        $files = ContentsFilesMapping::where('content_id',$book->id)->select('id')->get()->toArray();
        ...
        $images = $this->getImages($book->id);
        $files = $this->getDocs($book->id);
        return redirect()->route('admin.books.edit',compact('book','content_cats','files','images'));
    }
    public function doc_set_main(Book $book,File $file){
        $files = ContentsFilesMapping::where('content_id',$book->id)->select('id')->get()->toArray();
        File::whereIn('id',$files)
            ->where('type','doc')
            ->update([
    ...
    public function ajax_attachment_delete(Book $book,File $file){
        File::where('id',$file->id)->delete();
        ContentsFilesMapping::where('file_id',$file->id)->delete();
        \Illuminate\Support\Facades\File::delete(public_path('uploads/books/'.$book->id.'/').'/'.$file->file);
        echo '1';
    }
    ...
    private function getImages($content_id){
        return ContentsFilesMapping::where('contents_files_mappings.content_id',$content_id)
            ...
            ->where('contents_files_mappings.type','book')
            ->get();
    }
    ...
    public function update(Request $request, Book $book)
    {
        ...
        if($request->hasFile('image')){
            ...
            if(Storage::exists(public_path('uploads/books/'.$request->image))){
                Storage::delete(public_path('uploads/books/'.$request->image));
            }
            // upload new image
            $filename = md5($request->image->getClientOriginalName());
            $imageName = 'book_'.$filename.'.'.$request->image->extension();
       ...
       if ($request->images){
        ...
        return redirect()->route('admin.books.index')->with('success','رکورد با موفقیت ویرایش شد');
    }
    ...
    public function destroy(Book $book)
    {
        ...
        return redirect()->route('admin.books.index')->with('success','رکورد با موفقیت حذف شد');
    }
}
